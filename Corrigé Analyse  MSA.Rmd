...
ggplot(donnees_long, aes(x = Comparateur, y = Mesure, fill = Comparateur)) +
  ...
  theme_minimal()
```
**Analyse visuelle** : Le Boxplot montre que les médianes (le trait gras) des trois comparateurs ne sont pas parfaitement alignées. Le comparateur X semble avoir une moyenne légèrement négative, tandis que Y et Z sont centrés autour de zéro. L'ANOVA va nous dire si cet écart est statistiquement significatif ou simplement dû au hasard.
...
Puisque la $p_{value}$ > `r round(summary(modele_anova)[[1]][["Pr(>F)"]][1], 3)`, nous ne pouvons pas rejeter l'hypothèse nulle.
